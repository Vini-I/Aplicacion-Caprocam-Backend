...
export async function listarLotes(req, res) {
  try {
    ...
    const esGlobal = Boolean(
      user?.accesoGlobal || Number(user?.grupoDatos) === 22776226,
    );

    if (esGlobal && !req.query.grupoDatos) {
      const [rows] = await pool.query(
        `SELECT id, uuid, grupo_datos AS grupoDatos, codigo_lote AS codigoLote,
                        proveedor_id AS proveedorId, laboratorio_id AS laboratorioId,
                        procedencia_id AS procedenciaId, certificado_larva AS certificadoLarva,
                        pl_inicial AS plInicial, cantidad_inicial AS cantidadInicial,
                        fecha_ingreso AS fechaIngreso, estado_lote AS estadoLote, activo
                 FROM lotes_larva WHERE activo = TRUE AND deleted_at IS NULL`,
      );
      return exito(res, "Lotes de larva obtenidos correctamente.", rows);
    }
    ...
    if (esGlobal && !req.query.grupoDatos) {
      const [rows] = await pool.query(
        `SELECT id, uuid, grupo_datos AS grupoDatos, codigo_lote AS codigoLote,
                        ...
                        pl_inicial AS plInicial, cantidad_inicial AS cantidadInicial,
                        fecha_ingreso AS fechaIngreso, estado_lote AS estadoLote, activo
                 FROM lotes_larva WHERE id = ? AND activo = TRUE AND deleted_at IS NULL`,
        [req.params.id],
      );
      if (rows.length === 0)
        return error(res, "Lote de larva no encontrado.", null, 404);
      return exito(res, "Lote de larva obtenido correctamente.", rows[0]);
    }
...
export async function crearLote(req, res) {
  ...
  try {
    ...
    const laboratorioIdFinal =
      req.body.laboratorio_id ?? req.body.laboratorioId;
    const procedenciaIdFinal =
      req.body.procedencia_id ?? req.body.procedenciaId;

    const existente = await loteLarvaModel.findByCodigo(
      codigoLoteFinal,
      grupoDatos,
    );
    if (existente) {
    ...
    if (!isEmpty(proveedorIdFinal)) {
      const existe = await loteLarvaModel.verificarProveedorExiste(
        proveedorIdFinal,
        grupoDatos,
      );
      if (!existe) {
    ...
    if (!isEmpty(laboratorioIdFinal)) {
      const existe = await loteLarvaModel.verificarLaboratorioExiste(
        laboratorioIdFinal,
        grupoDatos,
      );
      if (!existe) {
    ...
    if (!isEmpty(procedenciaIdFinal)) {
      const existe = await loteLarvaModel.verificarProcedenciaExiste(
        procedenciaIdFinal,
        grupoDatos,
      );
      if (!existe) {
...
export async function actualizarLote(req, res) {
  ...
  try {
    ...
    const loteHeredado = await loteLarvaModel.esLoteHeredado(id, grupoDatos);
    if (loteHeredado) {
      return error(
        res,
        "No se puede actualizar este lote porque proviene de una Pre-Cría y ya generó una Siembra — sus datos quedan fijos por trazabilidad.",
        null,
        409,
      );
    }
